...
impl<PR: PartyRepository, TR: TeamRepository> EndPartyUseCase<PR, TR> {
    ...
    pub async fn execute(&self, dto: EndPartyDto) -> Result<(), (StatusCode, ApiErrorResponse)> {
        ...
        let team_winner = if let Some(team_id) = dto.team_winner_id {
            self.team_repository.get_by_id(team_id).await.map_err(|_| {
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
        ...
        let mut party = party.unwrap();

        if let Some(team) = &team_winner {
